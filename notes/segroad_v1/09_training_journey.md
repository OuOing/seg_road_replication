...
完整 validation threshold sweep 已完成，最终峰值出现在 `threshold=0.85`：

| Threshold | IoU | F1 | Precision | Recall | pred+ | target+ |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 0.650 | 0.4158 | 0.5874 | 0.4794 | 0.7582 | 0.0636 | 0.0402 |
| 0.700 | 0.4269 | 0.5983 | 0.5047 | 0.7346 | 0.0586 | 0.0402 |
| 0.750 | 0.4364 | 0.6076 | 0.5327 | 0.7071 | 0.0534 | 0.0402 |
| 0.800 | 0.4435 | 0.6145 | 0.5648 | 0.6738 | 0.0480 | 0.0402 |
| 0.850 | **0.4460** | **0.6169** | 0.6033 | 0.6311 | 0.0421 | 0.0402 |
| 0.875 | 0.4438 | 0.6147 | 0.6261 | 0.6038 | 0.0388 | 0.0402 |
| 0.900 | 0.4374 | 0.6086 | 0.6525 | 0.5703 | 0.0352 | 0.0402 |
| 0.925 | 0.4235 | 0.5951 | 0.6839 | 0.5266 | 0.0310 | 0.0402 |
| 0.950 | 0.3945 | 0.5658 | 0.7241 | 0.4643 | 0.0258 | 0.0402 |

这说明默认 `0.5` 的主要问题不是模型完全不会分割，而是后处理切点太宽松：`pred+` 从 0.0795 降到 0.0421 后，已经接近真实道路比例 0.0402，Precision 明显提升。代价是 Recall 下降，因此最终报告应同时列出默认阈值和 validation-selected threshold。
...
低学习率短程续训已完成。配置为从 epoch 8 best checkpoint 恢复，学习率覆盖为 `1e-4`，评估阈值使用完整 threshold sweep 得到的 `0.85`，训练到 epoch 12：

| Experiment | Epoch | Threshold | IoU | F1 | Precision | Recall | pred+ | target+ |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| epoch 8 threshold-selected | 8 | 0.85 | 0.4460 | 0.6169 | 0.6033 | 0.6311 | 0.0421 | 0.0402 |
| lr1e-4 finetune | 12 | 0.85 | **0.4779** | **0.6468** | 0.6839 | 0.6135 | 0.0361 | 0.0402 |

这说明短程续训有效：模型在更严格阈值下进一步减少误检，Precision 明显提高，IoU/F1 继续上升。代价是 Recall 小幅下降，`pred+` 低于 `target+`，说明预测更保守。下一步不建议盲目继续追加很多 epoch，应先做可视化对比，确认是否出现狭窄道路漏检增加。
...
PCS 消融已完成 8 个 epoch，最佳结果出现在 epoch 7：

| Experiment | Epoch | IoU | F1 | Precision | Recall | pred+ | target+ |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| baseline + PCS | 8 | 0.3776 | 0.5482 | 0.4128 | 0.8155 | 0.0795 | 0.0402 |
| no PCS (`alpha=0`) | 7 | 0.3729 | 0.5432 | 0.4128 | 0.7942 | 0.0774 | 0.0402 |

默认 `threshold=0.5` 下，PCS 对像素指标的提升不大，但 baseline 的 Recall 更高。训练早期差异更明显：no-pcs epoch 1 的 `pred+=0.2996`，比 baseline epoch 1 的 `0.2211` 更激进，说明缺少 PCS 后模型一开始更容易把大量线状背景判成道路。后续需要通过 prediction/error overlay 检查道路连通性差异。
